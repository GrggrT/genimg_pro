...
class ViewModel(QObject):
    ...
    def _run_task(self, func: Callable, *args: Any, on_finish: Callable) -> None:
        ...
        # !!! ШАГ 1: СИГНАЛ ОБ ОШИБКЕ УЖЕ ПОДКЛЮЧЕН ЗДЕСЬ !!!
    ...
    # ... (промежуточные методы без изменений)
    ...
    # !!! ШАГ 2: СЛОТ-ОБРАБОТЧИК ОШИБОК УЖЕ СУЩЕСТВУЕТ !!!
    @Slot(Exception)
    def _on_task_error(self, error: Exception) -> None:
        """
        Слот, который вызывается при ошибке в фоновом потоке.
        Показывает пользователю диалоговое окно с ошибкой.
        """
        error_message = str(error)
        print(f"[ОШИБКА] Произошла ошибка в фоновом потоке: {error_message}")
        
        # Показываем пользователю диалоговое окно
        self.main_window.show_error_message("Ошибка", error_message)
        
        # Обязательно разблокируем кнопку
        self.main_window.toggle_generate_button(True)