...
class ViewModel(QObject):
    """
    Асинхронный ViewModel, управляющий всей логикой приложения без блокировки GUI.

    Выступает в качестве связующего звена между View (MainWindow) и Model
    (CacheManager, ApiClient, ImageGenerator). Запускает длительные операции
    (сетевые запросы, генерация изображений) в отдельных потоках.
    """
    ...
    def __init__(self, main_window: MainWindow, cache_manager: CacheManager,
        ...
        Args:
            main_window (MainWindow): Экземпляр главного окна (View).
            cache_manager (CacheManager): Экземпляр менеджера кэша.
            api_client (ApiFootballClient): Экземпляр API-клиента.
            image_generator (ImageGenerator): Экземпляр генератора изображений.
        """
        super().__init__()
    ...
    def _find_team_flow(self, team_name: str, worker_progress_signal: Optional[Signal] = None) -> Dict[str, Any]:
        ...
        # ИЗМЕНЕНИЕ: Используем новый метод поиска по псевдониму
        ...
        # ИЗМЕНЕНИЕ: Используем новый метод для добавления/обновления команды
        self.cache_manager.add_or_update_team(
            team_name=api_data['name'],
            logo_filename=os.path.basename(logo_path),
            api_source='api-football',
            aliases=[team_name, api_data['name']] # Добавляем и поисковый запрос, и точное имя из API
        )
        
        # Повторяем поиск, чтобы получить только что созданную запись
        return self.cache_manager.find_team_by_alias(team_name)

    # --- Остальные методы (слоты-обработчики) остаются без изменений ---
